# Run transformer classification for ISCO codes
...
```{r define functions,message=FALSE}
source(here("exercises","R","helpers.R"))
source(here("exercises","R","helper_transformer.R"))
hparams <- fread(here("exercises","data","best_params.csv"))
```

```{r define hyperparameters}
...
FLAGS <- flags(
  ...
  flag_numeric("num_heads", hparams$num_heads), #number of transformer heads
  flag_numeric("num_transformer_blocks",hparams$num_trans_blocks), #number of transformer block
  flag_numeric("ff_dim",hparams$hidden_layer_dim),
  flag_numeric("maxlen",50),
  flag_numeric("embedding_dims",hparams$embed_dim),
  ...
  flag_integer("epochs",5), # maximum number of epochs
...
```{r prepare data}
# load data
source(here("exercises","R","load_data.R"))
...
# one hot encode 
#data.table with TokenID - Position in onehot matrix - IDF value
...
for(m in 1:max_epochs){
  ...
  for(step in 1:num_steps){
    ...
    y <- as.array(as.integer(dat[batch,][["ISCO_target"]]))
  ...
  names(eval_model) <- c("loss","accuracy","top_10_categorical_accuracy","top_5_categorical_accuracy")
  
  if(eval_model[[topk]]>current_best){
    current_best <- eval_model[[topk]]
    current_best_m <- m
    # uncomment if model should be saved for later use
...
for(step in 1:num_steps){
  ...
  eval_model_test <- model$evaluate(
                         x=x.test, 
                         y=y,
                         callbacks = callbacks)
...
names(eval_model_test) <- c("loss","accuracy","top_10_categorical_accuracy","top_5_categorical_accuracy")
